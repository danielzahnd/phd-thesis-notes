...
%	Second term in \cite{Baumann_2013}: \url{https://de.wikipedia.org/wiki/Elektromagnetische_Induktion}, \url{https://en.wikipedia.org/wiki/Electromagnetic_induction} and \url{https://en.wikipedia.org/wiki/Faraday%27s_law_of_induction} oder Faraday'sches Induktionsgesetz in Theoretische Physik s.16ff
	...
	\end{equation} Using Newton's second law and $q=-e$, one can derive the equation of motion \begin{equation}\label{eq:equation_of_motion_electrons_hall_effect}
...
\end{equation} 
Note, that the harmonic oscillator eigenstates are now dependent upon the wavenumber $k$. The wavenumber $k$ is a measure of the propagation speed of the eigenfunctions $\psi_{k,n}(x,y) \propto e^{iky}$, hence the eigenstates drift in the y-direction with group speed \begin{equation}\label{eq:qhe_state_drift}
	v_y = \frac{1}{\hbar}\frac{\partial E_{n,k}}{\partial k} = -\frac{E}{B}.
\end{equation} Hence the states drift in the direction of $\vect{E} \times \vect{B} = -EB\vect{e}_y$, as they move in the y-direction and the electric and magnetic fields are given by $\vect{E} = E\vect{e}_x$ and $\vect{B} = B\vect{e}_z$. More specifically, the drift velocity of states $\vect{v}_{d,s} = (0,-E/B, 0)^\top$ is given by $\vect{v}_{d,s} = B^{-2}\vect{E}\times\vect{B}$.
Now, considering again the wave functions associated to the eigenstates, the properly normalized wave function can only be obtained for a finite material of length $L_y$ in the y-direction as\begin{align}\footnotesize\begin{aligned}\label{eq:quantumhall_eigenfunctions_electricnonzero}
...
\subsection{Deriving the quantum Hall resistance}\label{sec:deriv_quantum_Hall}
...
\subsection{Observational conditions for the quantum Hall effect}
It remains to be answered, under which circumstances the quantum Hall effect is observable. The answer to this question is important to be able to actually build some experimental setup to realize a quantum Hall resistance standard.

Broadly speaking, there are five necessary conditions to be fulfilled in order to observe the quantum Hall effect, namely low temperature, strong magnetic field, two dimensional geometry, high mobility of electrons and sufficient disorder in the material. Each of these conditions will now be explained in more detail.

\subsubsection{Condition 1: Low temperature}
One of the most important conditions for the quantum Hall effect to arise is sufficiently low temperature $T$ of the quantum Hall system. The energy eigenstates of the quantum Hall system are just those of the quantum harmonic oscillator, given by \begin{equation}
	E_n = \hbar\omega_B\left(n+\frac{1}{2}\right), \quad n \in \mathbb{N}.
\end{equation} These energies are called Landau levels, which are hence proportional to the energy $\hbar\omega_B = \hbar eB/m_e$. The energy eigenstates mark the energy plateaus observable with the quantum Hall effect; thus they need to be well separated. However, if there is a non-zero temperature of the quantum hall system, electrons in the system will not only feature motion due to a magnetic and potentially an electric field, but also due to thermal energy, which is quantified by $k_B T$. The requirement, that the Landau levels need to be well separated in order for them to be observed, translates therefore to the expressions \begin{equation}\label{eq:qhe_existence_temp_cond}
k_B T \ll \hbar\omega_B = \hbar \frac{eB}{m_e} \quad \Leftrightarrow \quad  \frac{T}{B} \ll \frac{\hbar e}{m_ek_B} \quad \Leftrightarrow \quad T \ll \hbar\frac{eB}{m_ek_B}.
\end{equation} \cite{PhysRevLett.45.494} in their original paper give the magnetic field strength $B_{typ}$ for a typical quantum Hall experiment to be $B_{typ} \approx \SI{13}{\tesla}$ and the temperature $T_{typ}$ as $T_{typ} \approx \SI{1.8}{\kelvin}$. Now, using these data to arrive at an estimation of a critical temperature $T_{crit}$, one can calculate \begin{equation}
T_{crit} = \hbar\frac{eB_{typ}}{m_ek_B} \approx \SI{17.46}{\kelvin}.
\end{equation} As one can see, the temperature $T$ of a quantum Hall system needs to be much lower than $T_{crit}$, namely $T \ll T_{crit}$. The typical temperature noted by \cite{PhysRevLett.45.494} partially fulfills this requirement, as $T_{typ} < T_{crit}$.

\subsubsection{Condition 2: Strong magnetic field}
From the temperature conditions \cref{eq:qhe_existence_temp_cond} it is directly evident, that the ratio of $T/B$ needs to be minimized in order for the Landau energy states to be properly separated, which is needed for them to be observed. As $T$ is low and near zero, the ratio $T/B$ can be further minimized by maximizing the magnetic field strength $B$; thus, a high magnetic field $B$ is a necessary condition for the quantum Hall effect to be observed. 

However, the magnetic field can also get too strong for the Landau levels to be properly observed. This is the case when the magnetic field get so strong such that the Landau levels become widely spaces and thus overlap. A strong magnetic field has large amounts of energy stored in it; hence, electrons in the system moving according to \cref{eq:equation_of_motion_electrons_hall_effect} can gain sufficient energy to jump between Landau energy levels, thus leading to a breakdown of the well separated energy levels in the quantum Hall system.

\subsubsection{Condition 3: Two dimensional geometry}
The above given derivation \cref{sec:deriv_quantum_Hall} for the expression of the quantum Hall effect \cref{eq:quantumhalleffect} presupposes a number of experiment specifications. One of these requirements is that the geometry of the experiment is two dimensional; that is, the movement of the charge carriers is restricted to the x-y-plane. This requirement therefore has to be fulfilled to a best approximation in real-life implementations of a quantum Hall effect experiment.

\subsubsection{Condition 4: High mobility of electrons}
In quantum mechanics, the Heisenberg uncertainty relation \begin{equation}
	\Delta x \Delta p \geq \frac{\hbar}{2}
\end{equation} does apply for uncertainties $\Delta x$ in position and $\Delta p$ in momentum. This uncertainty principle is inherent to the fundamentals of quantum theory, the deeper reason being that if two operators - such as the position and momentum operators - do not commute with one another, it is impossible to construct a basis of simultaneous eigenstates of both operators. By the same principle, the so-called energy-time uncertainty relation \begin{equation}
\Delta E\Delta t \geq \frac{\hbar}{2}
\end{equation} can be derived, where $\Delta E$ is uncertainty in energy and $\Delta t$ uncertainty in time.

Now, the mobility of electrons in a quantum Hall system is related to the scattering time $\tau$ of the equation \cref{eq:equation_of_motion_electrons_hall_effect} of motion of electrons in the classical Hall effect system. The scattering time is a measure of the mobility of electrons insofar, as a higher scattering time means that more time between collisions of electrons passes. Therefore, if $\tau$ is high, one also has a large mobility; hence mobility is directly proportional to the scattering time $\tau$. Now, let $\Delta E$ denote the uncertainty inherent to the Landau energy levels of the quantum Hall effect and identify $\tau$ with the uncertainty in time $\Delta t$ of the energy-time uncertainty principle. Hence, one can derive \begin{equation}
	\Delta E \tau \propto \hbar \quad \Leftrightarrow \quad \Delta E \propto \frac{\hbar}{\tau}.
\end{equation} It is evident from this expression, that the higher the mobility of electrons in the system is, the lower the spread of Landau energy levels of the quantum Hall system will be. Thus at high electron mobility, the Landau energy levels are well defined and their width is such that they do not overlap, hence rendering the Landau energy plateaus well visible.

\subsubsection{Condition 5: Sufficient disorder}
Disorder due to impurities, defects or lattice imperfections of the quantum Hall sample leads to so-called localization of electron wave functions. Each Landau level has states of low energy spread, called the localized states; and states of large energy spread, called the extended states. If only extended states were present in the system, some energy states of a lower Landau level could overlap with a Landau level of higher energy, thus leading to not well defined Landau energy levels and hence to difficulties in making the quantum Hall effect visible in an experiment. Since disorder leads to localization of electron wave functions in the system, it contributes to the Landau energy levels to be well defined and separated. 

There however can also be to much disorder present in the material. Disorder can be quantified by a potential term $V(\vect{r})$ in the Hamiltonian operator of the quantum Hall system. Given that the Landau levels have an energy of order $\hbar \omega_B$, the potential simulating the disorder must be much smaller in order for the Landau levels to be still well separated. Therefore, the upper limit to disorder \begin{equation}
	V(\vect{r}) \ll \hbar \omega_B = \hbar \frac{eB}{m_e}
\end{equation} applies.

% Study 2.2.1 & 2.2.2 and baby version (p.43ff) in Tong lecture notes
% Also ask ChatGPT supplying the lecture notes
% Concerining temperature: Provide estimation based on comparison of
% k_B T << \hbar \omega_B for the thermic energy to be much less than
% the Landau plateaus. This is needed to observe the QHE!
% Argument about impurities for the plateaus to exist (for range of magnetic field same plateau)

\subsection{Edge modes}
The above treatment and derivation of the integer quantum Hall effect assumes, that a particle inside the system has zero potential energy. This is however not true in reality; especially at the edges of the material, the potential term in the Hamilton operator for a particle in the system is non-zero. This gives rise to so-called edge modes, which will be further explored in this subsection.


% Study 2.1.1 in Tong lecture notes
...
\subsection{Deriving the Josephson equations}
...
\subsection{Observational conditions for the Josephson effect}
The question remains, under which circumstances the Josephson effect can actually be observed. The answer to this question is important to be able to realize a Josephson voltage standard.

Broadly speaking, there are three necessary conditions to be fulfilled in order to observe the Josephson effect, namely low temperature, superconducting materials and a stable Josephson junction irradiation frequency. Each of these conditions will now be explained in more detail.

\subsubsection{Condition 1: Low temperature}
Text.

\subsubsection{Condition 2: Superconducting materials}
Text.

\subsubsection{Condition 3: Stable irradiation frequency}
Text.
...
% Go on with flux quantisation 3.7 in lecture notes.
...
In order to conduct measurements and thereby gather data to process, preparation of the experiment is necessary. There are several key steps to ensure, that the METAS Kibble balance is properly prepared and aligned to minimize possible error sources on the outcome of the experiment as well as possible. This will help to keep the uncertainty budget for the experiment as low as possible. The following material is concerned with a detailed description of the experiment preparation prior to any measurement.

\subsection{Step 1: Text}
Text.
...
\section{Double pendulum}
 % Explain, why the double pendulum for the Kibble balance is relevant.
